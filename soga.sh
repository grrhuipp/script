...
DeplaySoga() {
    ...
    if [ ! -z "$white_list_url" ]; then
        sed -i "/^white_list_url=/d" .env
        echo "white_list_url=$white_list_url" >>.env
    fi

    if [ ! -z "$routes_url" ]; then
        sed -i "/^routes_url=/d" .env
        echo "routes_url=$routes_url" >>.env
    fi
